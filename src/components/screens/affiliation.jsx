...
const TableAffiliation = ({darkmode}) => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch("https://jyssrmmas.pythonanywhere.com/api/affiliation/", {
                headers: {
    ...
    const fetchStudents = async () => {
        try {
            const response = await fetch("https://jyssrmmas.pythonanywhere.com/api/etudiants/", {
                headers: {
                    Authorization: `Bearer ${authToken}`,
                },
            });
    ...
    const fetchParents = async () => {
        try {
            const response = await fetch("https://jyssrmmas.pythonanywhere.com/api/Parentt/", {
                headers: {
                    Authorization: `Bearer ${authToken}`,
                },
            });
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space>
                    <Button icon={<EyeOutlined />} onClick={() => handleDetailView(record)} />
                </Space>
            ),
        },
    ];
    ...
    const handleBulkDelete = async () => {
        try {
            for (const key of selectedRowKeys) {
                await fetch(`https://jyssrmmas.pythonanywhere.com/api/affiliation/${key}/`, {
                    method: "DELETE",
                    headers: {
                        Authorization: `Bearer ${authToken}`,
                    },
        ...
        <div className="w-full p-2">
            ...
            >
                            ...
                            <>
                                ...
                                {selectedRowKeys.length === 1 && (
                                    <>
                                        <Button
                                            icon={<EditOutlined />}
                                            onClick={() => handleEditClick(data.find(item => item.id_affiliation === selectedRowKeys[0]))}
                                        >
                                        </Button>
                ...
                >
                    <Form form={form} layout="vertical">
                        ...
                        >
                            <Select>
                                {students.map((student) => (
                                    <Select.Option key={student.id_etudiant} value={student.id_etudiant}>
                        ...
                        <Form.Item
                            name="id_parent"
                            label="Parent"
                            rules={[{ required: true, message: "Veuillez sélectionner un parent" }]}
                        >
                            <Select>
                                {parents.map((parent) => (
                    ...
                    <Form layout="vertical">
                        ...
                        >
                            <Select
                                ...
                                onChange={(value) => setAffiliationData({ ...affiliationData, id_etudiant: value })}
                            >
                                {students.map((student) => (
                                    <Select.Option key={student.id_etudiant} value={student.id_etudiant}>
                                        {student.nom} {student.prenom}
                        ...
                        >
                            <Select
                                ...
                                onChange={(value) => setAffiliationData({ ...affiliationData, id_parent: value })}
                            >
                                {parents.map((parent) => (
                                    <Select.Option key={parent.id_parent} value={parent.id_parent}>
                                        {parent.nom} {parent.prenom}
                ...
                >
                    {detailAffiliation && (
                        <div>
                            <p><strong>ID Affiliation:</strong> {detailAffiliation.id_affiliation}</p>
                            <p><strong>Étudiant:</strong> {detailAffiliation.etudiant}</p>
                            <p><strong>Parent:</strong> {detailAffiliation.parent}</p>
                        </div>
                    )}
                </Modal>
            </ConfigProvider>
