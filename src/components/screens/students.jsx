...
const TableStudent = ({ darkmode }) => {
    ...
    const handleModalSubmit = async () => {
        try {
            ...
            values.date_inscription = values.date_inscription.format('YYYY-MM-DD');
            ...
            message.error("Une erreur s'est produite lors de la mise à jour de l'étudiant");
        }
    };
    ...
    const handleStudentSubmit = async () => {
        try {
            if (!studentData.nom || !studentData.prenom || !studentData.tel || !studentData.mail || !studentData.ville || !studentData.parent_id || !studentData.date_naissance || !studentData.date_inscription || !studentData.id_classe) {
                ...
                >
                    <Form form={form} layout="vertical">
                        ...
                        <Form.Item name="id_classe" label="Classe">
                            <Select>
                                {classes.map((classe) => (
                                    <Select.Option key={classe.id_classe} value={classe.id_classe}>
                        ...
                        <Form.Item name="parent_id" label="Parent">
                            <Select>
                                {parents.map((parent) => (
                                    <Select.Option key={parent.id_parent} value={parent.id_parent}>
                    ...
                    <Upload
                            ...
                            <Col span={12}>
                                <Form.Item label="Civilité" required>
                                    <Select
                                        ...
                                        onChange={(value) => setStudentData({ ...studentData, civilite: value })}
                                    >
                                        <Select.Option value="Monsieur">Monsieur</Select.Option>
                                        <Select.Option value="Madame">Madame</Select.Option>
                                        <Select.Option value="Mademoiselle">Mademoiselle</Select.Option>
                            ...
                            <Col span={12}>
                                <Form.Item label="Téléphone" required>
                                    <Input
                                        ...
                                        onChange={(e) => setStudentData({ ...studentData, tel: e.target.value })}
                                    />
                                </Form.Item>
                            </Col>
                            <Col span={12}>
                            ...
                            <Col span={12}>
                                <Form.Item label="Ville" required>
                                    <Select
                                        ...
                                        onChange={(value) => setStudentData({ ...studentData, ville: value })}
                                    >
                                        {cities.map((city) => (
                                            <Select.Option key={city.id} value={city.id}>
                                                {city.nom_ville}
                        ...
                        <Row gutter={16}>
                            <Col span={12}>
                                <Form.Item label="Date de naissance" required>
                                    <DatePicker
                                        className="w-full"
                                        value={studentData.date_naissance ? moment(studentData.date_naissance) : null}
                                        onChange={(date, dateString) => setStudentData({ ...studentData, date_naissance: dateString })}
                                    />
                                </Form.Item>
                            </Col>
                            <Col span={12}>
                                <Form.Item label="Date d'inscription" required>
                                    <DatePicker
                                        className="w-full"
                                        value={studentData.date_inscription ? moment(studentData.date_inscription) : null}
                                        onChange={(date, dateString) => setStudentData({ ...studentData, date_inscription: dateString })}
                                    />
                                </Form.Item>
                            </Col>
                        </Row>
                            ...
                            <Col span={12}>
                                <Form.Item label="Classe" required>
                                    <Select
                                        ...
                                        onChange={(value) => setStudentData({ ...studentData, id_classe: value })}
                                    >
                                        {classes.map((classe) => (
                                            <Select.Option key={classe.id_classe} value={classe.id_classe}>
                                                {`${classe.niveau} / ${classe.groupe}`}
                        ...
                        >
                            <Select
                                ...
                                onChange={(value) => setStudentData({ ...studentData, parent_id: value })}
                            >
                                {parents.map((parent) => (
                                    <Select.Option key={parent.id_parent} value={parent.id_parent}>
                                        {`${parent.nom} ${parent.prenom}`}
