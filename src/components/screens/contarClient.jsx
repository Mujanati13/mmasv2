...
import { Table, ConfigProvider, Input, Select, message, Popconfirm, Modal, Drawer, Button, Spin, DatePicker, Checkbox, Card, Row, Col, Divider } from 'antd';
import { SearchOutlined, DeleteOutlined, PrinterOutlined, FileAddOutlined, EyeOutlined, DownloadOutlined, PlusOutlined } from '@ant-design/icons';
import * as XLSX from 'xlsx';
import dayjs from 'dayjs';
import { addMonths, addNewTrace, getCurrentDate, toCapitalize } from '../../utils/helper';
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const desiredKeys = [
                    ...
                    { title: "Type d'Abonnement", dataIndex: 'abonnement', key: 'abonnement' },
                    { title: 'Réduction', dataIndex: 'reduction', key: 'reduction', render: (text) => `${text} MAD` },
                    { title: 'Type', dataIndex: 'type', key: 'type' },
                    { title: 'Date de Début', dataIndex: 'date_debut', key: 'date_debut' },
                    { title: 'Date de Fin', dataIndex: 'date_fin', key: 'date_fin' },
                    { title: "Catégorie d'Abonnement", dataIndex: 'cat_abn', key: 'cat_abn' },
                    {
                        title: 'Actions',
                        key: 'actions',
                        render: (_, record) => (
                            <EyeOutlined onClick={() => handleViewDetails(record)} style={{ cursor: 'pointer' }} />
                        ),
                    },
                ];
    ...
    const getDetailData = (contract) => {
        ...
        return [
            // { key: "section1", property: "Informations Personnelles", value: "" },
            {
                ...
                value: `${contract.client || ""} ${contract.Prenom_client || ""
                    }`.trim(),
            },
