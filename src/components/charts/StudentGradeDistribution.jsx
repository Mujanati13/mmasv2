...
const StudentGradeDistribution = ({ darkmode }) => {
    ...
    if (isLoading) return (
        <div className={`w-full h-96 ${darkmode ? 'bg-green-600' : 'bg-white'} 
    ...
    if (error) return (
        <div className={`w-full h-96 ${darkmode ? 'bg-green-600 text-red-400' : 'bg-white text-red-600'} 
    ...
    return (
        <div className={`w-full h-96 transition-colors duration-200 rounded-lg shadow-sm
            ${darkmode ? 'bg-green-600 border border-gray-700' : 'bg-white'}
            pt-5 pb-0`}
        >
            <h2 className={`font-medium text-center mb-0 transition-colors duration-200
